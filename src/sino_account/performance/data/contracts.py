...
def get_stock_unit(api: sj.Shioaji, code: str) -> float:
    """Return shares per lot for a stock code (default 1000)."""
    try:
        contract = resolve_stock_contract(api, code)
        unit = float(getattr(contract, "unit", 0) or 0)
        if unit > 0:
            return unit
    except Exception:
        pass
...
def build_code_units(api: sj.Shioaji, codes: list[str]) -> dict[str, float]:
    return {code: get_stock_unit(api, code) for code in codes}
