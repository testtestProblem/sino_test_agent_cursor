...
本文件描述 **sino-gui**（[`src/sino_account/gui/app.py`](../src/sino_account/gui/app.py)）的程式架構、模組依賴、執行緒模型、Shioaji API 對照，以及 **Get Positions 2** 的合併與市值演算法。

使用與操作說明見 [SINO_GUI.md](./SINO_GUI.md)。

**範圍**：Debug GUI 與其直接依賴的 `functions/`、`core/`、Positions 2 所用的 `performance/` 子集。**不含** `performance-3m`、`nav-chart-gui` 的完整架構（僅說明共用模組）。

---

## 2. 與現有專案的關係

`sino-gui` 是 `sino-account` 專案最早期的入口之一，後續績效與 NAV 圖表功能在其上擴充。
...
├── src/sino_account/
│   ├── core/                       # session、serialize（共用）
│   ├── functions/                  # login、get_*（GUI 按鈕）
│   ├── gui/
│   │   ├── app.py                  # sino-gui 主程式
│   │   └── nav_chart_app.py        # 獨立 CLI：nav-chart-gui
│   └── performance/                # Positions 2 部分引用
├── pyproject.toml
└── .env
```
...
flowchart TB
    ...
    subgraph perf_subset [performance 子集 Positions2]
...
僅複製下列檔案**無法**省略 `core/`；Positions 2 另需 `performance/` 三個模組。
...
| Functions | `functions/get_profit_loss.py` | 損益 |
| Functions | `functions/get_settlements.py` | 交割 |
| Core | `core/session.py` | API 連線、`.env` |
| Core | `core/serialize.py` | JSON 序列化、帳戶 label |
| Positions 2 | `performance/analytics/quantity_units.py` | 股數合併、市值 |
| Positions 2 | `performance/data/contracts.py` | 股票名稱、商品檔 |
| Positions 2 | `performance/data/throttle.py` | API 間隔 |
| 設定 | `.env` | API 憑證 |
...
**自訂成功回呼**（Get Positions 2）：回傳純文字，使用 `_set_text_output`。
...
| 函式 | 說明 |
|------|------|
| `load_env()` | 載入 `.env`（專案根目錄 `parents[3]` 自 `core/session.py` 推算） |
...
| 行為 | 說明 |
|------|------|
| `fetch_contract=True` | Login 時下載商品檔（Positions 2 解析股票名稱） |
...
帳務查詢速率限制（永豐）：約 **5 秒內 25 次**（見 [`sino_API_full.md`](../sino_API_full.md)）。Positions 2 在連續呼叫間使用 `throttle()`（0.25 秒）。
...
### 9.7 Get Positions vs Get Positions 2
...
Positions 2 在 `account_balance` → `list_positions(Common)` → `list_positions(Share)` 之間呼叫。
...
| 方法 | 使用時機 | 格式 |
|------|----------|------|
| `_set_output(payload)` | 多數按鈕 | `json.dumps(..., ensure_ascii=False, indent=2, default=str)` |
| `_set_text_output(text)` | Get Positions 2 | 固定寬度純文字 |
...
3. Handler 使用 `_run_api("action_name", lambda: ...)` 或自訂 `ApiWorker.submit`
